...
export default function PaymentMethods({ pgbo }: { pgbo?: any }) {
    ...
    return (
        <BaseLayout className="flex-col pt-4 pb-16">
                            ...
                            >
                                <CardContent className="flex flex-col flex-1 p-6 md:p-8">
                                    ...
                                    <div className="mt-auto relative z-10 w-full flex flex-col items-center">
                                        {isDisabled ? (
                                            <Button disabled className="w-full h-12 rounded-xl bg-slate-200 text-slate-400 font-bold cursor-not-allowed line-through hover:bg-slate-200">
                                                {method.cta}
                                            </Button>
                                        ) : (
                                            <Link
            ...
            <Dialog open={isPrintCostModalOpen} onOpenChange={setIsPrintCostModalOpen}>
                <DialogContent showCloseButton={false} className="sm:max-w-2xl max-h-[90vh] p-0 flex flex-col overflow-hidden bg-white rounded-3xl">
                    {/* Header Dialog */}
                    <DialogHeader className="px-6 py-4 border-b border-slate-100 flex flex-row items-center justify-between sticky top-0 bg-white/90 backdrop-blur z-20 mt-0 sm:mt-0">
                        <DialogTitle className="text-xl font-bold text-slate-800">Biaya Cetak</DialogTitle>
                        <DialogClose
                    ...
                    <div className="p-6 overflow-y-auto">
                        ...
                        <div className="mt-6 bg-blue-50 text-blue-800 p-4 rounded-xl text-sm leading-relaxed border border-blue-100 flex items-start gap-3">
                            💡
                            <p>Biaya cetak per keping, dibayar saat pengambilan di cabang atau pengiriman ke alamat Anda.</p>
                        </div>
                    </div>
                </DialogContent>
            </Dialog>
