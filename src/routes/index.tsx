      ...
      >
        <AnimatePresence>
          {!lockoutTime && !isUnlocked && (
            ...
            >
              <motion.div whileHover={{ scale: 1.05 }} className="p-3 bg-[#000856] rounded-2xl shadow-sm border border-slate-100">
        ...
        <AnimatePresence mode="wait">
          {!isUnlocked ? (
            ...
            <MotionCard
              key="auth-content" initial={{ opacity: 0, filter: 'blur(8px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }}
                ...
                <Tabs value={authMode} onValueChange={(v) => setAuthMode(v as 'signin' | 'signup')} className="w-full">
                  <div className="flex justify-center mb-4 pt-4">
                    <TabsList variant="line" className="flex bg-transparent border-none h-auto p-0 gap-8">
                      <TabsTrigger value="signin" className="font-bold rounded-none border-none py-3 text-xs transition-all px-2 text-slate-400 data-[state=active]:text-slate-900 data-[state=active]:after:!bg-slate-900">Masuk</TabsTrigger>
                      <TabsTrigger value="signup" className="font-bold rounded-none border-none py-3 text-xs transition-all px-2 text-slate-400 data-[state=active]:text-slate-900 data-[state=active]:after:!bg-slate-900">Daftar</TabsTrigger>
                    </TabsList>
                  </div>
                  <div className="p-6 sm:px-10 pb-8 pt-0">
                    <AnimatePresence mode="wait">
                      <TabsContent value="signin" className="mt-0 outline-none"><SignInForm /></TabsContent>
                      <TabsContent value="signup" className="mt-0 outline-none"><SignUpForm onSignupSuccess={() => setAuthMode('signin')} /></TabsContent>
                    </AnimatePresence>
                  </div>
                </Tabs>
                <div className="p-4 sm:p-5 pt-3 border-t border-slate-50 flex flex-col items-center gap-3 bg-transparent">
                  <a href="https://wa.me/628979901844" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
                    <div className="p-1.5 bg-emerald-50 rounded-full"><MessageCircle className="w-3.5 h-3.5 text-emerald-500" /></div>
        ...
        <AnimatePresence>
          {!lockoutTime && !isUnlocked && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col items-center gap-3 mt-4">
              <div className="flex items-center justify-center gap-2 opacity-30 text-[9px] text-slate-500"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Portal Keamanan Terpusat Public Gold</div>
